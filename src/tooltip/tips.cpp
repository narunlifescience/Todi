...
QString QTipLabel::helpId() const { return m_helpId; }

ColorTip::ColorTip(QWidget *parent) : QTipLabel(parent) { resize(40, 40); }
...
bool ColorTip::equals(int typeId, const QVariant &other,
                      const QString &otherHelpId) const {
  return typeId == ToolTip::ColorContent && otherHelpId == helpId() &&
         other == m_color;
}
...
bool TextTip::isInteractive() const { return likelyContainsLink(m_text); }
...
void TextTip::configure(const QPoint &pos, QWidget *w) {
  ...
  int extraHeight = 0;
  if (fm.descent() == 2 && fm.ascent() >= 11) ++extraHeight;
...
bool TextTip::equals(int typeId, const QVariant &other,
                     const QString &otherHelpId) const {
  return typeId == ToolTip::TextContent && otherHelpId == helpId() &&
