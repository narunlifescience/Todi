...
DirViewItemDir::DirViewItemDir(const QString name, DirViewItem *parent)
    : DirViewItem(name, DirViewItem::Type_Dir), d_parentItem(parent) {}

DirViewItemDir::~DirViewItemDir() { qDeleteAll(d_childItems); }

int DirViewItemDir::row() const {
  switch (d_parentItem->type()) {
    case DirViewItem::Type_Root:
      return static_cast<DirViewItemRoot *>(d_parentItem)
          ->d_childItems.indexOf(const_cast<DirViewItemDir *>(this));
    case DirViewItem::Type_Dir:
      return static_cast<DirViewItemDir *>(d_parentItem)
          ->d_childItems.indexOf(const_cast<DirViewItemDir *>(this));
    case DirViewItem::Type_File:
      qWarning() << "Unhandled Type DirViewItem::Type_File";
      break;
  }
...
DirViewItem *DirViewItemDir::parent() const { return d_parentItem; }

int DirViewItemDir::childCount() const { return d_childItems.count(); }
