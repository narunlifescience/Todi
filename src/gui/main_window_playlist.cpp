...
void MainWindow::setupPlaylistViewMenu() {
  ...
  connect(albumPlaylistViewAction, SIGNAL(toggled(bool)), this,
          SLOT(playListTableViewToggleAlbum(bool)));
  connect(artistPlaylistViewAction, SIGNAL(toggled(bool)), this,
          SLOT(playListTableViewToggleArtist(bool)));
  connect(timePlaylistViewAction, SIGNAL(toggled(bool)), this,
          SLOT(playListTableViewToggleTime(bool)));
  connect(trackPlaylistViewAction, SIGNAL(toggled(bool)), this,
          SLOT(playListTableViewToggleTrack(bool)));
  connect(titlePlaylistViewAction, SIGNAL(toggled(bool)), this,
          SLOT(playListTableViewToggleTitle(bool)));
  connect(discPlaylistViewAction, SIGNAL(toggled(bool)), this,
          SLOT(playListTableViewToggleDisc(bool)));
}

void MainWindow::setupPlaylistViewHeader() {
  playlistTableViewHeader = playlistTableView->horizontalHeader();

  playlistTableViewHeader->setSectionsMovable(true);
  playlistTableViewHeader->setSectionResizeMode(QHeaderView::Stretch);
  playlistTableViewHeader->setContextMenuPolicy(Qt::CustomContextMenu);

  connect(playlistTableViewHeader, SIGNAL(customContextMenuRequested(QPoint)),
          this, SLOT(playlistTableViewContextMenuClicked()));
  connect(playlistTableViewHeader, SIGNAL(sectionMoved(int, int, int)), this,
          SLOT(playListTableViewSaveState()));
  connect(playlistTableViewHeader, SIGNAL(sectionResized(int, int, int)), this,
          SLOT(playListTableViewSaveState()));
}
...
void MainWindow::playListTableViewSaveState() {
  QSettings settings;
  settings.setValue("playlistTableViewHeader",
                    playlistTableViewHeader->saveState());
}
