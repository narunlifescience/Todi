...
dirViewModel::dirViewModel(QObject *parent)
    : QAbstractItemModel(parent), rootItem(new DirViewItemRoot("")) {}

dirViewModel::~dirViewModel() { delete rootItem; }

QModelIndex dirViewModel::index(int row, int column,
...
QModelIndex dirViewModel::parent(const QModelIndex &index) const {
  ...
  const DirViewItem *const childItem =
      static_cast<DirViewItem *>(index.internalPointer());
...
QVariant dirViewModel::headerData(int /*section*/,
