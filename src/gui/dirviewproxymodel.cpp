...
bool DirViewProxyModel::lessThan(const QModelIndex &left,
                                 const QModelIndex &right) const {
  const DirViewItem *const leftItem =
      static_cast<DirViewItem *>(left.internalPointer());
  const DirViewItem *const rightItem =
      static_cast<DirViewItem *>(right.internalPointer());
