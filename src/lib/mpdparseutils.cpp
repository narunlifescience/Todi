...
void MPDParseUtils::parseStatus(const QByteArray *const data) {
  ...
  for (int i = 0; i < amountOfLines; i++) {
    ...
    if (tokens.at(0) == "volume") {
      status->setVolume(static_cast<quint8>(tokens.at(1).toUInt()));
      ...
      if (tokens.at(1).contains("play")) {
        status->setState(MPDStatus::State::StatePlaying);
      } else if (tokens.at(1).contains("stop")) {
        status->setState(MPDStatus::State::StateStopped);
      } else {
        status->setState(MPDStatus::State::StatePaused);
      }
      ...
      status->setBitrate(static_cast<quint16>(tokens.at(1).toUInt()));
...
Song *MPDParseUtils::parseSong(const QByteArray *const data) {
  ...
  for (int i = 0; i < amountOfLines; i++) {
    ...
    if (element == "file") {
      ...
      song->pos = static_cast<quint32>(value.toInt());
    } else if (element == "Id") {
      song->id = static_cast<qint32>(value.toUInt());
...
QList<MusicLibraryItemArtist *> *MPDParseUtils::parseLibraryItems(
  ...
  for (int i = 0; i < amountOfLines; i++) {
    ...
    if (i == lines.size() - 1 || lines.at(i + 1).startsWith("file:")) {
      ...
      songItem->setTrack(static_cast<quint32>(currentSong->track));
...
DirViewItemRoot *MPDParseUtils::parseDirViewItems(
    const QByteArray *const data) {
  QList<QByteArray> lines = data->split('\n');
  ...
  for (int i = 0; i < amountOfLines; i++) {
    ...
    if (line.startsWith("file: ")) {
      ...
      if (currentDir->type() == DirViewItem::Type_Root)
        static_cast<DirViewItemRoot *>(currentDir)
            ->insertFile(parts.at(parts.size() - 1));
      else
        static_cast<DirViewItemDir *>(currentDir)
            ->insertFile(parts.at(parts.size() - 1));
      ...
      if (currentDir->type() == DirViewItem::Type_Root)
        currentDir = static_cast<DirViewItemRoot *>(currentDir)
                         ->createDirectory(parts.at(parts.size() - 1));
      else
        currentDir = static_cast<DirViewItemDir *>(currentDir)
                         ->createDirectory(parts.at(parts.size() - 1));
...
QString MPDParseUtils::seconds2formattedString(const quint32 totalseconds) {
  ...
  QTime time(static_cast<int>(hours), static_cast<int>(minutes),
