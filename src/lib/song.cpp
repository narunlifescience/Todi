...
Song::Song() : QObject(), id(-1), time(0), track(0), pos(0), disc(0) {}
...
bool Song::isEmpty() const {
  return ((artist.isEmpty() && album.isEmpty() && title.isEmpty()) ||
          file.isEmpty());
}
...
ScrobblingSong::ScrobblingSong(const ScrobblingSong &s)
    : Song(s), timePlayedAt(s.timePlayedAt) {}
