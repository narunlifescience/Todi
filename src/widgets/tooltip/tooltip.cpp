...
void ToolTip::show(const QPoint &pos, const QString &content, QWidget *w,
  ...
  if (content.isEmpty())
    ...
    instance()->showInternal(pos, QVariant(content), TextContent, w, helpId,
                             rect);
}
...
void ToolTip::show(const QPoint &pos, const QColor &color, QWidget *w,
  ...
  if (!color.isValid())
    ...
    instance()->showInternal(pos, QVariant(color), ColorContent, w, helpId,
                             rect);
}
...
void ToolTip::show(const QPoint &pos, QWidget *content, QWidget *w,
  ...
  if (!content)
    ...
    instance()->showInternal(pos, QVariant::fromValue(content), WidgetContent,
                             w, helpId, rect);
}
...
void ToolTip::move(const QPoint &pos, QWidget *w) {
  if (isVisible()) instance()->placeTip(pos, w);
}
...
bool ToolTip::acceptShow(const QVariant &content, int typeId, const QPoint &pos,
                         QWidget *w, const QString &helpId, const QRect &rect) {
  if (isVisible()) {
  ...
  // While the effect takes places it might be that although the widget is
  // actually on
  // screen the isVisible function doesn't return true.
  else if (m_tip && (QApplication::isEffectEnabled(Qt::UI_FadeTooltip) ||
...
void ToolTip::setUp(const QPoint &pos, QWidget *w, const QRect &rect) {
  ...
  if (m_hideDelayTimer.isActive()) m_hideDelayTimer.stop();
...
bool ToolTip::tipChanged(const QPoint &pos, const QVariant &content, int typeId,
  ...
  if (!m_tip->equals(typeId, content, helpId) || m_widget != w) return true;
  if (!m_rect.isNull()) return !m_rect.contains(pos);
  return false;
}
...
void ToolTip::hide() { instance()->hideTipWithDelay(); }

void ToolTip::hideImmediately() { instance()->hideTipImmediately(); }

void ToolTip::hideTipWithDelay() {
  if (!m_hideDelayTimer.isActive()) m_hideDelayTimer.start(300);
}
  ...
  switch (event->type()) {
    ...
    case QEvent::KeyRelease:
      if (HostOsInfo::isMacHost()) {
        ...
        Qt::KeyboardModifiers mody =
            static_cast<QKeyEvent *>(event)->modifiers();
        if (!(mody & Qt::KeyboardModifierMask) && key != Qt::Key_Shift &&
    ...
    case QEvent::FocusIn:
      if (m_tip && !m_tip->isInteractive())  // Windows: A sequence of those
    ...
    case QEvent::Wheel:
      if (m_tip) {
        if (m_tip->isInteractive()) {  // Do not close on interaction with the
                                       // tooltip
